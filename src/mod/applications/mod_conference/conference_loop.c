		...
		if (switch_channel_test_app_flag(channel, CF_APP_TAGGED)) {
			...
			if ((write_frame.datalen = (uint32_t) switch_buffer_read(use_buffer, write_frame.data, bytes))) {
				if (write_frame.datalen) {
					...
					//lifeng: 没有talking, 就不要将此人的rtp包发送出去
					/*send = FALSE;
					for (omember = member->conference->members; omember; omember = omember->next) {
						if (conference_utils_member_test_flag(omember, MFLAG_TALKING)) {
							send = TRUE;
							break;
						}
					}*/
					//if (send) {
					//switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_ERROR, "发送rtp.....");
